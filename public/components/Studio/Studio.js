...
export default class Studio extends React.Component {
  ...
  handleCreateSample = () => { console.log("create sample"); };

  handleCreateWorkspace = () => { console.log("create workspace"); };

  handleLoadWorkspace = index => { console.log("load workspace", index); };
  ...
  handlePlayWorkstation = (editingBeat) => { console.log("play"); }

  handleUploadImage = () => { console.log("upload image"); }

  handleNameChange = (value) => { console.log(value); }

  handleSaveBeat = (editingBeat) => { console.log("save beat") }
  ...
  renderActivators =
      ...
      return (
        <div
      key = {`${index}${single.isActive}`} className =
          "SWActivatorContainer" > < img
      src = {single.isActive ? checkGreen : checkRed} height = "25px"
      width = "25px"
  ...
  renderGridItems =
      ...
      return (
        <Rnd
      key = {`${index}${workItem.row}${workItem.col}`} className = "SWGridItem"
        style={
        style}
        default={
        { x: 0, y: 40 * workItem.row, width: 40 * workItem.length, height: 40 }}
        minHeight={40}
        maxHeight={40}
