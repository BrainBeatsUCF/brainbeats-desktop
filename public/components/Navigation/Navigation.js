...
export default class Navigation extends React.Component {
  ...
  renderButton(title, src, height, width, target, isActive, isLogout) {
    return (
      <div className={`NavigationInput ${isActive ? "NavigationInputActive": ""} ${isLogout ? "NavigationLogoutInput" : ""}`} onClick={target}>
        <label>{title}</label>
    ...
    return (
      <div className="NavigationBackground">
        <div className="NavigationBackgroundLeft">
          {this.renderButton(
            "Home", HomeImage, 18, 22, 
            () => this.handleSwitchToHome(), 
            this.state.isDisplayingHomePage, false
          )}
          {this.renderButton(
            "Studio", StudioImage, 18, 22, 
            () => this.handleSwitchToStudio(), 
            !this.state.isDisplayingHomePage, false
          )}
        </div>
        {this.renderButton(
          "Logout", LogoutImage, 18, 19, 
          () => this.handleLogout(), 
          false, true
        )}
      </div>
